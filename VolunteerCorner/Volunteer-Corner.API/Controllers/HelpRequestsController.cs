...
public class HelpRequestsController : ControllerBase
{
    ...
    [Authorize(Roles = CustomRoles.VolunteerRole)]
    public async Task<IActionResult> AddResponse(string id, [FromBody] AddHelpRequestResponseRequest request)
    {
        var volunteerId = User.FindFirstValue(CustomClaimTypes.VolunteerId);
            
        if (string.IsNullOrWhiteSpace(volunteerId))
            return Forbid();

        var result = await _helpRequestService.CreateResponseAsync(id, volunteerId, request);
        return StatusCode(StatusCodes.Status201Created, result);
    }
}